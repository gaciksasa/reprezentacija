...
// User profile routes - available for authenticated users
...
Route::middleware(['auth', 'can:admin'])->group(function () {
    ...
    Route::post('tim-varijante/postavi-glavni-tim/{tim}', [TimVarijanteController::class, 'postaviGlavniTim'])
        ->name('tim-varijante.postaviGlavniTim');
});

// Editor/Admin routes (editing content)
Route::middleware(['auth', 'can:edit'])->group(function () {
    ...
    // Tim varijante routes (except setting main team, which is admin-only)
    ...
    // CRUD for categories
    Route::resource('kategorije', KategorijaController::class)->except(['index', 'show']);

    // CRUD for opponent coaches
    Route::resource('protivnicki-selektori', ProtivnickiSelektoriController::class)->except(['index', 'show']);

    // CRUD for stadiums
    Route::resource('stadioni', StadioniController::class)->except(['index', 'show']);

    // CRUD for competitions
    Route::resource('takmicenja', TakmicenjaController::class)->except(['index', 'show']);

    // CRUD for referees
    Route::resource('sudije', SudijeController::class)->except(['index', 'show']);
...
// Read-only routes (for all users including guests)
...
Route::get('/kategorije/{kategorije}', [KategorijaController::class, 'show'])->name('kategorije.show');
